...
//        vec3f interpNorm = light->tris[randTri]->interpolateNormal(randBCs);
		vec3f randDir = scene->returnSurfaceSamplePoint(&randBCs, &randTri) - randPos;
        
		Ray photonray(randPos + randDir * offset, randDir);
...
//        return;
//    }
//    vec3f max = vec3f(-INFINITY,-INFINITY, -INFINITY);
//    vec3f min = vec3f(INFINITY,INFINITY, INFINITY);
//
...
//        tempColor = tempColor + tempphoton->color * 1.0f /(static_cast<float>(NumPhotons)*unitArea);
//    }
    return tempColor;
}
